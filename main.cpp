...
//#define UseSettings
const auto encryptedAddress = cryptor::create("play.uifserver.net"); unsigned short port = 7776;
...
void InitSAMP()
{
	Log("DescaApk Samp loaded! Build time: " __DATE__ " " __TIME__);
...
void InitNetwork()
{
	if(!bNetworkInited && pSettings->Get().iFontOutline)
	{
		...
		#else
			pNetGame = new CNetGame(encryptedAddress.decrypt(), port, pSettings->Get().szNickName, "");
		#endif
