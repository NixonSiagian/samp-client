...
void InitGame(RPCParameters *rpcParams)
{
	...
	bsInitGame.ReadCompressed(pNetGame->m_bLanMode);								// 
...
void RequestSpawn(RPCParameters *rpcParams)
{
	...
	if(pPlayerPool) 
	{
		...
		if(pLocalPlayer)
		{
			if(byteRequestOutcome == 2 || (byteRequestOutcome && pLocalPlayer->m_selectionClassData.m_bWaitingForSpawnRequestReply))
				pLocalPlayer->Spawn();
			else pLocalPlayer->m_selectionClassData.m_bWaitingForSpawnRequestReply = false;
		}
	}
}
...
#define REJECT_REASON_BAD_NICKNAME  0
